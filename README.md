...
3. **代码执行**：集成火山引擎 ARK API，为设计场景生成并执行代码，AI 生成的代码块支持直接编辑后执行
...
私服模式提供独立的 Token 计费与用户管理体系，支持以下高级特性：

- **双模式套餐共存**：纯Token套餐（无时限、可累加购买）与限时套餐（day/week/month/quarter/year）可同时存在
- **Token扣减优先级**：优先消耗限时套餐Token，耗尽后再扣减纯Token套餐额度
- **多模式登录**：基于URL uinfo参数的UUID模式 + 基于用户名+密码的手动模式（URL无uinfo时优雅降级）
- **安全哈希传输**：用户密码与管理员密钥均在前端 SHA-256 哈希后传输，杜绝明文传输
- **头像下拉菜单**：用户页面右上角提供「修改密码」与「登出」功能入口
- **多模型管理**：支持动态多模型配置与智能降级切换

详见 `ai_schematic_private/README.md`
...
对话界面右侧提供六栏抽屉面板，点击顶部标签页一键切换：

- **图谱**：D3 力导向图，可视化展示 AI 工具调用链与元件关联关系，支持搜索节点并自动居中
- **提示词**：预置 14 个提示词模板（系统提示词、工作流模板、业务场景模板等），点击即可插入对话
- **精选工具**：展示 33 个可用工具（27 个自定义工具 + 6 个 MCP 元工具），支持搜索过滤与直接执行
- **EDA函数**：嘉立创 EDA 原生 API 完整列表，方便查阅与调用
- **资源**：管理本地导入的工具与提示词数据，支持导入/导出
- **日志**：每次 AI 对话的请求与响应完整记录，JSON 内容支持按层级折叠展开

![抽屉面板](images/drawer.gif)

### 知识图谱

图谱面板以力导向图形式呈现 AI 工具调用关系与元件关联网络：

- 节点大小反映引用强度，连线粗细区分强/弱引用
- 点击节点高亮关联路径（品红色描边）
- 搜索时唯一匹配节点自动居中定位
- 悬浮节点可查看详细信息
...
- **导入默认**：加载内置工具与提示词数据
- **从源码导入**：从外部 JSON 文件导入自定义工具或提示词
- **导出**：将当前数据导出为 JSON 文件备份
- 数据存储于 IndexedDB，关闭插件后持久保留
