...
//
// 【数据角色说明】本文件是「出厂默认 1.0」提示词数据的载体;
// 当本地数据库(IndexedDB)已激活时,data-store.js 会用数据库数据覆盖 window.promptList,
                ...
                "content": {
                    ...
                    "text": `
                        ...
                        - component_placement：
                          ...
                          * 规则：元件放置前必须获取画布大小，确保不超出边界；元件间距必须符合业务规则（调用 @{business_rules_spacing} 获取画布-元件间距、元件间距、导线间距等精确数值）；批量放置时，多个元件应一起放置，减少操作次数；subPartName 必填，即使为空字符串（不能省略）；x, y 必须在画布范围内
                        ...
                        - collision_detection：
                          ...
                          * 规则：必须使用统一图元碰撞检测机制检测所有碰撞；新元件与画布边界、其他元件边界、现有导线的距离均须符合业务规则（调用 @{business_rules_spacing} 获取精确数值：画布-元件、元件间、导线间间距等）；批量放置时，还需检测新元件之间的相互距离（同元件间距规范）；确保无碰撞或已记录违规项；如有碰撞，执行移动元件操作；移动元件后重新执行碰撞检测，最多重试3次
                ...
                "content": {
                    ...
                    "text": `
                        ...
                        - obstacle_analysis：
                          ...
                          * 规则：必须获取画布大小；必须获取所有元件列表及其引脚坐标；必须获取各个元件的边界信息（通过引脚列表计算边界）；必须获取所有现有导线的路径信息；构建障碍物地图：元件边界及安全区域（通过引脚列表计算出来的矩形边界区域，禁止导线穿越）、现有导线路径及安全区域（每条导线路径向外扩展的安全距离，禁止导线穿越）；所有间距数值以 @{business_rules_spacing} 为准
                          * 检查点：障碍物分析完成（必需）- 已获取所有元件列表及其引脚坐标、元件边界信息、现有导线路径信息，已构建障碍物地图
                        
                        - path_calculation_collision：
                          * 推荐工具：无（纯算法计算）
                          * 规则：使用A*算法进行路径搜索，在障碍物地图上进行计算；必须先根据元件的引脚列表获取边界再进行计算；实时碰撞检测（路径计算过程中必须检查，使用统一图元碰撞检测机制）：新导线路径与画布边界、元件边界、其他导线、引脚的距离均符合规范（具体数值见 @{business_rules_spacing}，须用边界矩形计算，不能只看引脚坐标）；识别90°拐点，替换为两段45°走线；算法参数：距离权重 + 拐点罚分 + 碰撞罚分（碰撞时路径成本大幅增加）；如有碰撞，调整路径或重新规划，最多重试3次；纯算法计算，不调用工具
                ...
                "content": {
                    ...
                    "text": `
                        ...
                        - spec_validation：
                          ...
                          * 规则：验证所有间距标准（必须检查所有间距，具体数值见 @{business_rules_spacing}：画布-元件、画布-导线、元件-元件边界、元件-导线边界、导线-导线、导线-元件边界、导线-引脚等间距）；验证布局规划：功能分组（按电源/信号/控制/接口等模块集中摆放）、流向布局（按输入→处理→输出的信号流向摆放）、网络标签优先（相同网络标签优先用标签替代跨图直线）
        ...
        "messages": [
            {
                ...
                "content": {
                    ...
                    "text": `
                        ...
                        - 确保元件间距符合规范（具体数值见 @{business_rules_spacing}）
                        - 考虑后续布线需求，预留足够空间
                        `
                }
            }
        ...
        "messages": [
            {
                ...
                "content": {
                    ...
                    "text": `
                        ...
                        检测规则（具体数值见 @{business_rules_spacing}，以下为检测维度与例外说明）：
                        ...
                        - 所有间距数值以 @{business_rules_spacing} 为准，若二者冲突以该规则为准
        ...
        "messages": [
            {
                ...
                "content": {
                    ...
                    "text": `
                        ...
                        - 工作流定义不再硬编码枚举：请先调用 listPrompts 获取当前可用的工作流与规则清单，
                          再按需通过 getPrompt({name:'workflow_xxx'}) 拉取对应工作流定义（如：
                          workflow_requirement_analysis、workflow_component_design、workflow_wiring_design、workflow_validation_optimization）
